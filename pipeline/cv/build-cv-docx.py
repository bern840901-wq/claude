...
# ── docx helpers ──
def set_font(run, size=8.5, color=TAUPE, bold=False, name='Arial'):
...
def tcmar(cell, top=40, bottom=40, left=80, right=80):
    tcPr = cell._tc.get_or_add_tcPr()
...
def row_rule(tbl, ri, color=RULE):  # thin bottom border on a row's cells
    for c in tbl.rows[ri].cells:
        tcPr = c._tc.get_or_add_tcPr()
        b = OxmlElement('w:tcBorders'); e = OxmlElement('w:bottom')
        e.set(qn('w:val'), 'single'); e.set(qn('w:sz'), '4'); e.set(qn('w:color'), color)
        b.append(e); tcPr.append(b)

def para(cell_or_doc, text=None, size=8.5, color=TAUPE, bold=False, align=None,
         before=0, after=0, line=None):
    p = cell_or_doc.paragraphs[0] if hasattr(cell_or_doc, 'paragraphs') and \
        hasattr(cell_or_doc, '_tc') and not cell_or_doc.paragraphs[0].runs else \
        (cell_or_doc.add_paragraph() if hasattr(cell_or_doc, 'add_paragraph') else cell_or_doc)
...
def section_header(doc, title, before=11, after=5):
    p = doc.add_paragraph(); p.paragraph_format.space_before = Pt(before)
    p.paragraph_format.space_after = Pt(after)
    r1 = p.add_run('▌ '); set_font(r1, 11.5, TEAL, True)
    r2 = p.add_run(title); set_font(r2, 11.5, NAVY, True)
    return p
...
def build(lang):
    ...
    for m in ('top_margin', 'bottom_margin'): setattr(sec, m, Cm(1.05))
    for m in ('left_margin', 'right_margin'): setattr(sec, m, Cm(1.25))
    doc.styles['Normal'].font.name = 'Arial'
    doc.styles['Normal'].paragraph_format.space_after = Pt(0)

    # ── banner: navy card, name left / contact + QR right ──
    ban = doc.add_table(rows=1, cols=3); no_borders(ban)
    fixed_widths(ban, [5960, 2400, 1000])
    c0, c1, c2 = ban.rows[0].cells
    shade(c0, BAN_L); shade(c1, BAN_R); shade(c2, BAN_R)
    tcmar(c0, 160, 160, 170, 80); tcmar(c1, 160, 160, 80, 40); tcmar(c2, 90, 90, 20, 90)
    vcenter(c0); vcenter(c1); vcenter(c2)
    p = c0.paragraphs[0]; p.paragraph_format.space_after = Pt(2)
    r = p.add_run(U['sub']); set_font(r, 18, 'FFFFFF', True)
    para(c0.add_paragraph(), U['subsub'], 8.6, BAN_TXT, after=4)
    para(c0.add_paragraph(), strip(L(S)['role']), 8.2, BAN_TXT, line=1.3)
    for i, t in enumerate([U['loc'], PHONE, EMAIL]):
        para(c1.add_paragraph() if i else c1.paragraphs[0], t, 8.8, 'FFFFFF' if i else BAN_TXT,
             bold=(i > 0), after=3)
    pq = c2.paragraphs[0]; pq.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    pq.add_run().add_picture(f'{OUT}/qr.png', width=Cm(2.1))

    # ── stats strip ──
    st = doc.add_table(rows=1, cols=4); no_borders(st)
    fixed_widths(st, [2340, 2340, 2340, 2340])
    for i, s0 in enumerate(L(S)['stats']):
        c = st.rows[0].cells[i]; shade(c, 'F5F0E6'); tcmar(c, 90, 90, 80, 80); vcenter(c)
        p = c.paragraphs[0]; p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.paragraph_format.space_after = Pt(0)
        r = p.add_run(s0['num'] + ' '); set_font(r, 13.5, NAVY, True)
        r = p.add_run(s0['label']); set_font(r, 9, NAVY, True)
        para(c.add_paragraph(), s0['sub'], 7.4, TAUPE, align=WD_ALIGN_PARAGRAPH.CENTER)

    # ── profile (3 paragraphs per skill) ──
    section_header(doc, U['profile'])
    para(doc, strip(L(S)['lede']), 9.2, NAVY, bold=True, after=4, line=1.35)
    para(doc, strip(L(S)['aboutDesc']), 9, TAUPE, after=4, line=1.35)
    para(doc, strip(L(S)['heroTrust']), 9, TAUPE, line=1.35)

    # ── long-term partners ──
    section_header(doc, U['partners'])
    pt = doc.add_table(rows=len(D['PARTNERS'][lang]), cols=3); no_borders(pt)
    fixed_widths(pt, [1250, 4100, 4010])
    for i, pr in enumerate(D['PARTNERS'][lang]):
        cells = pt.rows[i].cells
        for c in cells: tcmar(c, 34, 34, 40, 40)
        para(cells[0], None); cells[0].paragraphs[0].add_run(pr['period'])
        set_font(cells[0].paragraphs[0].runs[0], 8.4, TEAL, True)
        r = cells[1].paragraphs[0].add_run(pr['org']); set_font(r, 9, INK, True)
        r = cells[2].paragraphs[0].add_run(pr['desc']); set_font(r, 8.4, TAUPE)

    # ── selected cases ──
    section_header(doc, U['work'])
    tb = doc.add_table(rows=len(rows) + 1, cols=4); no_borders(tb)
    fixed_widths(tb, [800, 5660, 1300, 1600])
    for j, h in enumerate(U['th']):
        c = tb.rows[0].cells[j]; shade(c, 'EFE8D8'); tcmar(c, 30, 30, 50, 50)
        r = c.paragraphs[0].add_run(h); set_font(r, 8.2, NAVY, True)
    row_rule(tb, 0, 'B8A88C')
    hdrPr = tb.rows[0]._tr.get_or_add_trPr()
    th_el = OxmlElement('w:tblHeader'); hdrPr.append(th_el)

    for tr in tb.rows:
        trPr = tr._tr.get_or_add_trPr()
        cs_el = OxmlElement('w:cantSplit'); trPr.append(cs_el)
    for i, cs in enumerate(rows, start=1):
        cells = tb.rows[i].cells
        for c in cells: tcmar(c, 44, 44, 50, 50); vcenter(c)
        y = '2016–17' if cs['y'] == 2016 and 'YICFFF' in cs['zh'] else str(cs['y'])
        r = cells[0].paragraphs[0].add_run(y); set_font(r, 8.2, TEAL, True)
        r = cells[1].paragraphs[0].add_run(cs[lang]); set_font(r, 8.5, INK, True)
        cells[1].paragraphs[0].paragraph_format.line_spacing = 1.18
        r = cells[2].paragraphs[0].add_run(FORM[cs['t']][lang]); set_font(r, 8, TAUPE)
        fg, bg = CAT[cs['cat']]
        shade(cells[3], bg)
        p3 = cells[3].paragraphs[0]; p3.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p3.add_run(CATL[cs['cat']][lang]); set_font(r, 7.8, fg, True)
        row_rule(tb, i)
    para(doc, U['full'], 8, TEAL, bold=True, before=5, align=WD_ALIGN_PARAGRAPH.RIGHT)

    # ── clients (page 2 filler + substance) ──
    section_header(doc, U['clients'])
    ct4 = doc.add_table(rows=1, cols=4); no_borders(ct4)
    fixed_widths(ct4, [2340, 2340, 2340, 2340])
    for gi, grp in enumerate(D['CLIENTS']):
        c = ct4.rows[0].cells[gi]; tcmar(c, 30, 30, 40, 60)
        p = c.paragraphs[0]; p.paragraph_format.space_after = Pt(3)
        r = p.add_run(grp['title'][lang]); set_font(r, 8.4, NAVY, True)
        for it in grp['items']:
            p = c.add_paragraph(); p.paragraph_format.space_after = Pt(1.5); p.paragraph_format.line_spacing = 1.12
            r = p.add_run(strip(it['name'][lang])); set_font(r, 7.9, INK, it['tier'] == 1)
            sub = strip(it['sub'][lang])
            if sub:
                r = p.add_run('  ' + sub); set_font(r, 7.2, TAUPE)

    # ── languages / credentials / education ──
    section_header(doc, U['edu'])
    bt = doc.add_table(rows=1, cols=2); no_borders(bt)
    fixed_widths(bt, [4680, 4680])
    left, right = bt.rows[0].cells
    for c in (left, right): tcmar(c, 40, 40, 50, 80)
    p = left.paragraphs[0]; p.paragraph_format.space_after = Pt(2)
    r = p.add_run(U['langsT']); set_font(r, 9.2, NAVY, True)
    for lg in D['LANGS']:
        p = left.add_paragraph(); p.paragraph_format.space_after = Pt(2); p.paragraph_format.line_spacing = 1.15
        r = p.add_run(strip(lg['name'][lang]) + ' — '); set_font(r, 8.6, INK, True)
        r = p.add_run(strip(lg['level'][lang])); set_font(r, 8.2, TAUPE)
    p = left.add_paragraph(); p.paragraph_format.space_before = Pt(4); p.paragraph_format.space_after = Pt(2)
    r = p.add_run(U['certsT']); set_font(r, 9.2, NAVY, True)
    for ct in D['CERTS'][lang]:
        p = left.add_paragraph(); p.paragraph_format.space_after = Pt(2); p.paragraph_format.line_spacing = 1.15
        r = p.add_run(ct['name'] + ' — '); set_font(r, 8.6, INK, True)
        r = p.add_run(ct['issuer']); set_font(r, 8.2, TAUPE)
    p = right.paragraphs[0]; p.paragraph_format.space_after = Pt(2)
    r = p.add_run(U['eduT']); set_font(r, 9.2, NAVY, True)
    for ed in D['EDU']:
        p = right.add_paragraph(); p.paragraph_format.space_after = Pt(2); p.paragraph_format.line_spacing = 1.15
        r = p.add_run(ed['year'] + '　'); set_font(r, 8.2, TEAL, True)
        r = p.add_run(ed['school'][lang] + '　'); set_font(r, 8.4, TAUPE)
        r = p.add_run(ed['deg'][lang]); set_font(r, 8.4, INK, True)
        if ed['note'][lang]:
            p = right.add_paragraph(); p.paragraph_format.space_after = Pt(2)
            r = p.add_run('　　' + strip(ed['note'][lang])); set_font(r, 7.8, TAUPE)
